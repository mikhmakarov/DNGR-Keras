...
def learn_embeddings(A, d=16, ksteps=10, alpha=0.98, hidden_layers=3):
    hidden_neurons = [128, 64, d]
    adj_matrix = np.asarray(A.todense())

    data = random_surfing(adj_matrix, ksteps, alpha)
    data = PPMI_matrix(data)

    embds = model(data,hidden_layers,hidden_neurons, 'dngr_out.txt')
